<?php
include 'header.php';
if (isset($_SESSION['user']))
  if ($_SESSION['admin'] == 1)
    header('Location: dashboard.php');


?>

<link rel="stylesheet" href="style.css" />
<link href="https://fonts.googleapis.com/icon?family=Material+Icons" rel="stylesheet" />
<link rel="stylesheet" href="https://cdn.jsdelivr.net/npm/bootstrap-icons@1.3.0/font/bootstrap-icons.css">


<script src="https://code.jquery.com/jquery-3.6.0.js"></script>
<script src="https://code.jquery.com/ui/1.13.1/jquery-ui.js"></script>
<script>
  $(function() {
    $("#tabs").tabs();
  });
</script>
</head>

<body>
  <!-- header starts -->
  <div class="header">
    <div class="header__left">
      <a class="navbar-brand col-md-3 col-lg-2 me-0 px-3" href="home.php">SocialContract.com</a>

      <!-- <div class="header__input">
          <span class="material-icons"> search </span>
          <input type="text" placeholder="Search Facebook" />
        </div> -->
    </div>
    <div class="header__right">
      <div class="header__info">

        <?php if (isset($_SESSION['user'])) { ?>

          <a class="nav-link px-3 " href="request.php">
            <span class='bi bi-person-x-fill' style='color:#000; height:25px'></span>
            <?php if (isset($_SESSION['request'])) { ?>
              <span class="position-absolute  translate-middle badge rounded-pill bg-danger">
                <?php echo (count($_SESSION['request'])); ?>
              </span>
          <?php }
          }
          ?>
          </a>
          <a class="nav-link px-3 text-info" href="find.php">Search for Friends!</a>

          <img class="user__avatar" src="data:image/png;base64," alt="" />
          <a href="friends.php" style="text-decoration:none" class="text-muted"> <?php echo "<span class='mx-2' >" . $_SESSION['username'] . " </span>"; ?></a>

      </div>

      <a href="logout.php"> <span class="material-icons"> logout </span></a>
    </div>
  </div>

  <br>
  <main>

    <div style="width: 45%;" class="mx-auto">

      <div class="input-group">
        <input type="search" class="form-control rounded" placeholder="Search by Username or Email-id" aria-label="Search" aria-describedby="search-addon" id="findVal" />
        <button type="button" class="btn btn-outline-primary" id="find">search</button>
        <button class="btn btn-info float-end" id='showAll'>Show All</button>

      </div>

      <br>
    </div>


    <div class="container marketing " style="width: 50%;">
      <hr>
      <hr>

      <!-- Three columns of text below the carousel -->
      <div class="row">

        <?php
        if (!isset($_SESSION['findFriend'])) {
          try {
            $id = $_SESSION['id'];


            $sql = "SELECT * FROM User where user_id NOT IN 
            (SELECT friend_id
            FROM friend where user_id='$id') AND
            user_id NOT IN 
            (SELECT user_id
            FROM request where request_id='$id')";
            $res = $conn->query($sql);

            $rows = $res->fetchAll(PDO::FETCH_ASSOC);

            // echo(count($rows));

          } catch (PDOException $e) {
            echo "Connection failed: " . $e->getMessage();
          } ?>
          <?php
          $id = $_SESSION['id'];
          foreach ($rows as $key => $val) {
            if ($id != $val['user_id']) { ?>
              <div class="col-lg-12 col-md-12 col-sm-12 row" id="searchFriends">

                <div class="col">
                  <img src="<?php echo ($val['picture'])   ?>" class="border" style="border-radius: 25px;" alt="" width="50%">
                  <br><br>
                </div>
                <div class="col">
                  <span class="h5" style="font-size:2.2vw"><?php echo ($val['fname'] . ' ' . $val['lname']);   ?></span>
                  <br><span class="h6" style="font-size:2vw">(<?php echo ($val['username']);   ?>)</span>
                </div>
                <div class="col">
                  <p><button class="btn btn-primary float-end addFriend" href="#" id="<?php echo ($val['user_id'])   ?>" onclick="this.innerHTML='Request Sent';this.css('color','black')" style="font-size:1.5vw">Add Friend</button></p>

                </div>
              </div>
              <br>
              <hr>

          <?php
            }
          }
        } else if ($_SESSION['findFriend'] !== 0) { ?>
          <div class="col-lg-12 row" id="searchFriends">

            <div class="col">
              <span><img src="<?php echo ($_SESSION['findPicture']);   ?>" class="border" style="border-radius: 25px;" alt="" width="50%"></span>
              <br><br>
            </div>
            <div class="col">
              <h3><?php echo ($_SESSION['findFriend']);   ?></h3>
              <h5>(<?php echo ($_SESSION['findusername']);   ?>)</h5>
            </div>
            <div class="col">
              <p><a class="btn btn-primary float-end addFriend" href="#" id="<?php echo ($_SESSION['findFriendId']);   ?>" onclick="this.innerHTML='Request Sent'">Add Friend</a></p>

            </div>
          </div>
          <br>
          <hr>


        <?php  } else {
          echo ("No user Found");
        }
        ?>
      </div><!-- /.row -->
    </div><!-- /.container -->
  </main>
  <footer class="text-muted py-5">
    <div class="container">
      <p class="float-end mb-1">
        <a href="#">Back to top</a>
      </p>
    </div>
  </footer>

</body>

</html>